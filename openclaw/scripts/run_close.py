...
def fetch_close(base_url: str, use_mock: bool) -> dict[str, Any]:
    opener = _build_opener()
    close_url = f"{base_url.rstrip('/')}/v1/review/close"
    ...
    if not api_response.get("ok"):
        raise RuntimeError(f"close API returned not ok: {api_response}")
    ...
    return {
        ...
        "warnings": warnings,
        "close_api_response": api_response,
    }


def _default_log_path() -> Path:
    project_root = Path(__file__).resolve().parents[2]
    return project_root / ".data" / "close_runs.jsonl"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run close fetch/writeback helpers.")
    parser.add_argument(
        "--mode",
        default="fetch",
...
def main() -> int:
    ...
    try:
        if args.mode == "fetch":
            result = fetch_close(base_url=args.base_url, use_mock=use_mock)
        else:
        ...
        try:
            ...
            print(f"close log write failed: {log_exc}", file=sys.stderr)
        ...
        try:
            ...
            pass
        print(f"close {args.mode} 执行失败: {exc}", file=sys.stderr)
        return 1
